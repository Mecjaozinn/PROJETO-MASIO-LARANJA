...
perguntas = {
    ...
    "perg_media": [
        {"texto": "Em qual cidade ocorreu o caso Araceli Crespo?", "opcoes": ["Vitória", "Belo Horizonte", "Salvador", "Recife"], "resp": "A"},
...
def receber_denuncia():
    ...
    conn = sqlite3.connect(DB_PATH)
    cur  = conn.cursor()
    cur.execute("INSERT INTO denuncias (tipo, descricao, local) VALUES (?, ?, ?)",
                (tipo, descricao, local))
    conn.commit()
    conn.close()
    return jsonify({"mensagem": "Denúncia registrada! Obrigado por agir."}), 201

# ══════════════════════════════════════
# API — QUIZ: sortear perguntas
# ══════════════════════════════════════
...
def iniciar_quiz():
    ...
    for nivel in niveis:
        ...
        sessao.append({"nivel": nivel, "chave": chave, "idx": idx,
                        "texto": p["texto"], "opcoes": p["opcoes"]})
    return jsonify(sessao)

# ══════════════════════════════════════
# API — QUIZ: corrigir e salvar ranking
# ══════════════════════════════════════
...
def resultado_quiz():
    ...
    for r in respostas:
        chave    = r["chave"]
        idx      = r["idx"]
        resp_usr = r["resposta"].upper()
        correta  = perguntas[chave][idx]["resp"]
        acertou  = resp_usr == correta
        if acertou:
            nota += 1
        detalhes.append({"texto": perguntas[chave][idx]["texto"],
                          "resposta_usuario": resp_usr,
                          "resposta_correta": correta,
                          "acertou": acertou})
    conn = sqlite3.connect(DB_PATH)
    cur  = conn.cursor()
    cur.execute("INSERT INTO ranking (nome, nota, tempo) VALUES (?, ?, ?)",
                (nome, nota, tempo))
    conn.commit()
    conn.close()
    return jsonify({"nota": nota, "total": len(respostas),
                    "tempo": round(tempo, 2), "detalhes": detalhes})
...
def get_ranking():
    ...
    rows = cur.fetchall()
    conn.close()
    return jsonify([{"nome": r[0], "nota": r[1], "tempo": r[2]} for r in rows])
